...
plot_species_presence <- function(species, space, col = NULL) {
  ...
  conditional_plot(
    title = paste0("species_presence_", species$id), # title
    plot_fun = plot_single, # plot_fun
    space = space, # space
    values = all_presence, # presence vector
    title2 = paste("Species", species[["id"]]),
    col = col
  )
}
...
plot_species_abundance <- function(species, space, col = NULL) {
  ...
  conditional_plot(
    title = paste0("species_abundance_", species$id),
    plot_fun = plot_single,
    space = space,
    values = all_presence,
    title2 = paste("Abundance Species", species[["id"]]),
    col = col
  )
}
...
plot_space_overview <- function(space, env_names = NULL, breaks = NULL) {
  ...
  x_breaks <- seq(
    space$meta$area$extent[["xmin"]],
    space$meta$area$extent[["xmax"]],
    length.out = 7
  )
  y_breaks <- seq(
    space$meta$area$extent[["ymin"]],
    space$meta$area$extent[["ymax"]],
    length.out = 7
  )
...
#' @noRd
plot_space_overview.raster <- function(
  env_names,
  env_vars,
  breaks,
  x_breaks,
...
#' @returns no return value, called for plot
#' @noRd
plot_space_overview.h3 <- function(
  space,
  env_names,
  env_vars,
  breaks,
  ...
  plot_list <- lapply(env_names, function(vari) {
    ...
    polygons <- wrap_dateline_h3(
      polygons,
      space,
      conditional = (space$meta$area$extent[["xmin"]] >= -180 &
        space$meta$area$extent[["xmin"]] <= -170) &
        (space$meta$area$extent[["xmax"]] <= 180 &
          space$meta$area$extent[["xmax"]] >= 170)
    )
...
#' @returns no return value, called for plot
#' @noRd
plot_space_overview.points <- function(
  env_names,
  env_vars,
  breaks,
  x_breaks,
...
plot_summary <- function(output, summary_title = NULL, summary_legend = NULL) {
  ...
  {
    layout(matrix(
      c(1, 3, 3, 1, 3, 3, 1, 3, 3, 2, 2, 2, 2, 2, 2),
      ncol = 3,
      byrow = TRUE
    ))
    #layout.show(3)
    ...
    if (is.null(summary_legend)) {
      ...
      summary_legend <- paste(
        ...
        paste(
          ...
          paste0(round(sumar$occupancy[length(sumar$occupancy)], 1) * 100, "%"),
          sep = ": "
        ),
        paste("initial_richness", phylo[1, "total"], sep = ": "),
        paste("cumulative_richness", phylo[2, "total"], sep = ": "),
        paste("richness_present", phylo[2, "alive"], sep = ": "),
        paste(
          "speciation",
          paste0(
            round(
              (col_sum["speciations"] - phylo["initial", "total"]) /
                phylo[1, "total"] *
                100,
              0
            ),
        ...
        paste(
          "extinction",
          paste0(
            round((col_sum["extinctions"]) / phylo[1, "total"] * 100, 0),
            "%"
          ),
          sep = ": "
        ),
        paste(
          names(output$system)[1],
          round(output$system$`runtime-hours`, 2),
          sep = ": "
        ),
        sep = "\n"
      )
    ...
    legend(
      "topleft",
      inset = c(0, -0.4),
      title = paste0("Summary [", summary_title, "]"),
    ...
    legend(
      x = 1,
      y = max(d[, c("speciations", "extinctions")]),
    ...
    if (max_ras == 0) {
      ...
      if (min_ras == 0) {
        #if there is zero-richness (i.e. inhabited sites)
...
plot_ranges <- function(species_list, space, disturb = 0, max_sps = 10) {
  ...
  alive <- unlist(lapply(species_list, function(x) {
    length(x$abundance)
  }))
  alive <- alive > 0
  ...
  spp_list <- lapply(1:n_sps_max, function(i) {
    sp_coords <- space[["coordinates"]][
      names(species_list[[i]][["abundance"]]),
      ,
      drop = FALSE
    ]
  ...
  p <- base_plot + # background
    ggplot2::geom_jitter(
      # the species symbols (shape and color)
      data = spp_list,
...
conditional_plot <- function(title, plot_fun, ...) {
  ...
  fun_calls <- sys.calls()
  if (
    any(sapply(fun_calls, FUN = function(x) {
      is(x[[1]], "name") && "call_main_observer" == x[[1]]
    }))
  ) {
    ...
    #                 units = "px",
    #                 dpi = 300)
    ratio <- (space$extent[["ymax"]] - space$extent[["ymin"]]) /
      (space$extent[["xmax"]] - space$extent[["xmin"]])
    ggplot2::ggsave(
      filename = file_name,
      plot = p,
      width = (1400 / (ratio)),
...
plot_single <- function(no_data = 0, legend = TRUE, ...) {
  ...
  UseMethod("plot_single", args$space)
}
...
#' @param legend logical. TRUE or FALSE
#' @param ... other arguments dispatched to the method:
#' \itemize{
#'  \item space: a space to plot the values onto
...
#'  \item title2: a string with plot title
#'  \item col: a vector with colors
#' }
#'
...
#'
#' @noRd
plot_single.gen3sis_space_points <- function(no_data = 0, legend = TRUE, ...) {
  space <- list(...)$space
  values <- list(...)$values
  title <- list(...)$title2
  col <- list(...)$col
  ...
  spatial_points <- sf::st_as_sf(
    as.data.frame(space$coordinates),
    coords = c("x", "y")
  )
...
#'
#' @noRd
plot_single.gen3sis_space_h3 <- function(no_data = 0, legend = TRUE, ...) {
  space <- list(...)$space
  values <- list(...)$values
  title <- list(...)$title2
  col <- list(...)$col
...
plot_multiple <- function(no_data = 0, legend = TRUE, ...) {
  ...
  # Assuming 'x' is the first argument and we need the second for dispatch,
  # which is the first in 'args'
  UseMethod("plot_multiple", args$space)
}
...
#'
#' @noRd
plot_multiple.gen3sis_space_raster <- function(
  no_data = 0,
  legend = TRUE,
  ...
) {
  ...
  img <- matrix(
    ...
    ncol = ncol(values) + 2,
    dimnames = list(
      rownames(space[["coordinates"]]),
      c(colnames(space[["coordinates"]]), colnames(values))
    )
  )
...
#'
#' @noRd
plot_multiple.gen3sis_space_h3 <- function(no_data = 0, legend = TRUE, ...) {
  space <- list(...)$space
  values <- list(...)$values
  col <- list(...)$col
...
#'
#' @noRd
plot_multiple.gen3sis_space_points <- function(
  no_data = 0,
  legend = TRUE,
  ...
) {
...
check_args <- function(args) {
  ...
  if (!"space" %in% names(args)) {
    stop(
      "No gen3sis_space provided for dispatch as second parameter, please make sure you add one"
    )
  }
}
...
#' @noRd
.default_raster_plot_aesthetics <- function(
  space,
  col,
  x_breaks,
  y_breaks,
...
wrap_dateline_h3 <- function(
  polygons,
  space,
  conditional = (space$extent[["xmin"]] >= -180 &
    space$extent[["xmin"]] <= -170) &
    (space$extent[["xmax"]] <= 180 & space$extent[["xmax"]] >= 170)
) {
  if (conditional) {
    # wrap dateline
