...
simulation_timeframe <- function(
  ...
  scale_time <- (conv_unit(1, from = space_tunit, to = time_unit) *
    space_timestep) /
    step_time
  ...
  if (time_unit != space_tunit) {
    message(
      paste0(
        'Space time unit is "',
        space_tunit,
        '" but config time unit is "',
        time_unit,
        '".\n',
        '1 ',
        space_tunit,
        ' = ',
        conv_unit(1, from = space_tunit, to = time_unit),
        ' ',
        time_unit,
        '\n',
        'Each space timestep comprises ',
        space_timestep,
        ' ',
        space_tunit,
        '\n',
        'Specified config values will be mutiplied by ',
        scale_time,
        'to cope with space.'
      )
    )
  ...
  message(c(
    "The following will be implicitly time-scaled:",
...
config_interpreter <- function(config_file) {
  ...
  if (!any(grepl("^step_time", human_config))) {
    ...
    human_config <- c(
      human_config[1:new_line - 1],
      "",
      human_config[new_line:length(human_config)]
    )
  ...
  human_config[sim_lines] <- gsub(
    "\\bscale_time\\b",
    "config$user$scale_time",
    human_config[sim_lines]
  )
