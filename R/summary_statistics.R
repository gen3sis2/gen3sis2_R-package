...
init_summary_statistics <- function(data, vars, config) {
  ...
  phylo_summary <- matrix(
    NA,
    nrow = 1,
    ncol = 4,
    dimnames = list(
      "initial",
      c("total", "alive", "speciations", "extinctions")
    )
  )

  phylo_summary[1, "total"] <- length(data$all_species)
  phylo_summary[1, "alive"] <- sum(unlist(lapply(data$all_species, function(x) {
    ifelse(length(x$abundance), 1, 0)
  })))
  phylo_summary[1, "speciations"] <- length(data$all_species)
  phylo_summary[1, "extinctions"] <- phylo_summary[1, "total"] -
    phylo_summary[1, "alive"]

  data$summaries[["phylo_summary"]] <- phylo_summary

  data$summaries[["occupancy"]] <- c(
    "initial" = percentage_inhabited(data$all_species, data$space)
  )
...
update_summary_statistics <- function(data, vars, config) {
  ...
  alive <- sum(unlist(lapply(data$all_species, function(x) {
    ifelse(length(x$abundance), 1, 0)
  })))
  ...
  occupancy <- c(
    data$summaries$occupancy,
    percentage_inhabited(data$all_species, data$space)
  )
  names(occupancy) <- tmp
...
make_summary <- function(config, data, vars, total_runtime, save_file = TRUE) {
  ...
  #summary
  final_richness <- cbind(
    data[["inputs"]][["coordinates"]],
    rep(0, nrow(data[["inputs"]][["coordinates"]]))
  )
  colnames(final_richness) <- append(
...
write_runtime_statisitics <- function(data, vars, config, total_runtime) {
  ...
  stat_file <- file(file.path(
    config$directories$output,
    "runtime_information.txt"
  ))
  sink(stat_file)
...
percentage_species_alive <- function(species_list) {
  alive <- sum(sapply(species_list, function(sp) {
...
habitat_extents <- function(
  species_list,
  exclude_extinct = TRUE,
  plot_hist = FALSE
) {
  extents <- sapply(species_list, function(sp) {
    length(sp[["abundance"]])
  })
  names(extents) <- sapply(species_list, function(sp) {
    as.character(sp[["id"]])
  })
  if (exclude_extinct) {
