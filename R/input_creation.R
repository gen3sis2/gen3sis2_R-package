...
create_spaces_raster <- function(
  raster_list, # old spaces
  cost_function,
  directions = 16,
  output_directory,
  # timesteps = NULL,
  ...
  compiled_env <- compile_spaces(raster_list, timesteps, habitability_masks)
  ...
  for (step in 1:nts) {
    ...
    distance_local <- get_local_distances(
      space_stack,
      habitable_mask,
      cost_function,
      directions,
...
compile_spaces <- function(spaces, timesteps, habitability_masks) {
  ...
  for (i in seq_along(timesteps)) {
    ...
    habitability <- get_habitable_mask(habitability_masks, space_stack, i) |>
      as.vector()
...
get_local_distances <- function(
  ...
  ref_raster <- terra::rast(
    ext = ext,
    resolution = rs,
    crs = terra::crs(space_stack),
    vals = 1:prod(dim(space_stack)[1:2])
  )
  ...
  trans_matrices <- get_transition_correction(
    x = ref_raster,
    tr_fun = function(x) {
      x[1]
    },
  ...
  transition_matrix <- drop0(transition_matrix) *
    trans_matrices$correctionMatrix
...
get_habitable_mask <- function(habitability_masks, space_stack, i) {
  if (is.null(habitability_masks)) {
    ...
    stop(
      "unknown habitability_mask; it has to be null, a list of rasters, or paths to raster files"
    )
  }
...
stack_spaces <- function(spaces, i) {
  ...
  for (space in spaces) {
    if (is.character(space[i])) {
      ...
      stop(
        "Unknown space; it must be a named list of list of either rasters or raster files"
      )
    }
    suppressWarnings(
...
create_directories <- function(directory, overwrite, full_matrices) {
  ...
  dir.create(
    file.path(directory, "distances_local"),
    recursive = TRUE,
    showWarnings = FALSE
  )
  if (full_matrices) {
    dir.create(
      file.path(directory, "distances_full"),
      recursive = TRUE,
      showWarnings = FALSE
    )
  }
