...
loop_speciation <- function(config, data, vars) {
  ...
  for (spi in 1:vars$n_sp) {
    ...
    if (config$user$needs_scaling[["get_divergence_factor"]]) {
      ...
      ifactor_exceeds_threshold <- any(
        ifactor > config$gen3sis$speciation$divergence_threshold
      )
      ...
      if (ifactor_exceeds_threshold && !threw_warning) {
        ...
        threw_warning <- TRUE
        warning(paste0(
          "Cumulative genetic distance is up to",
          signif(
            max(ifactor) / config$gen3sis$speciation$divergence_threshold,
            3
          ),
