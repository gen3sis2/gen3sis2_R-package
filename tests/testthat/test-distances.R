...
test_that("distance matrix creation works", {
  ...
  ras <- raster(
    nrows = num_cells,
    ncols = num_cells,
    vals = 1 + runif(num_cells * num_cells)
  )
  ras <- terra::rast(
    nrows = num_cells,
    ncols = num_cells,
  ...
  local_distance <- get_local_distances(
    spaces,
    h_mask,
    function(src, h_src, dest, h_dest) {
      src
    },
    8,
    NULL
  )
  dist_m <- get_distance_matrix(
    habitable_cells = 1:prod(dim(ras)),
