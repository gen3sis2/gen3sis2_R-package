...
save_val <- function(val, save_state = NA) {
  ...
  if (create_save) {
    ...
    if (!dir.exists(state_dir)) {
      # if the saving directory does not exist
...
save_ecogengeo <- function(val) {
  ...
  if (is.null(val$vars$ti)) {
    ti <- val$config$gen3sis$general$duration$from
  } else {
    ti <- val$vars$ti
  }
  ...
  saveRDS(
    object = species,
    file = file.path(
      val$config$directories$output_species,
      paste0("species_t_", val$vars$ti, ".rds")
    )
  )
  ...
  saveRDS(
    object = space,
    file = file.path(
      val$config$directories$output_spaces,
      paste0("space_t_", val$vars$ti, ".rds")
    )
  )
}
