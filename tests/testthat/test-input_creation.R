...
test_that("create_spaces_raster works properly", {
  ...
  withr::with_tempdir({
    ...
    files_expected <- c(
      "distances_full/distances_full_0.rds",
...
test_that("compile_spaces works", {
  # create rasters
  ...
  compiled_spaces <- compile_spaces(
    spaces = spaces,
    timesteps = timesteps,
  ...
  expect_true(all.equal(
    colnames(compiled_spaces[["l1"]]),
    c("x", "y", timesteps)
  ))
  expect_true(all.equal(
    colnames(compiled_spaces[["l2"]]),
    c("x", "y", timesteps)
  ))
  expect_true(all(
    terra::values(r11) ==
      terra::values(terra::rast(
        compiled_spaces[["l1"]][, c("x", "y", 1)],
        type = "xyz"
      ))
  ))
  expect_true(all(
  ...
  expect_true(all(
    terra::values(r22) ==
      terra::values(terra::rast(
        compiled_spaces[["l2"]][, c("x", "y", 2)],
        type = "xyz"
      ))
  ))
  expect_true(all.equal(
    rownames(compiled_spaces[["l1"]]),
...
test_that("get_local_distances works", {
  ...
  space_stack <- stack_spaces(spaces, 1)
  habitable_mask <- get_habitable_mask(
    habitability_masks = NULL,
    space_stack,
    1
  )
...
test_that("create_directories overwrite protection works", {
  ...
  local_mock(file.exists = function(...) {
    return(TRUE)
  })
  expect_error(
...
test_that("create_directories works", {
  # mocking base functions is no longer feasible, overwrite is not tested as it calls "unlink"
  new_dirs <- list()
  local_mocked_bindings(dir.create = function(new_dir, ...) {
    new_dirs <<- append(new_dirs, new_dir)
  })
