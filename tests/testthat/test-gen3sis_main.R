...
test_that("simulation loop breaks if the species limits are reached", {
  ...
  withr::with_tempdir({
    ...
    expect_true(
      "[!] Max number of species reached, breaking loop " %in% the_function_said
    )
  })
  ...
  withr::with_tempdir({
    ...
    expect_true(
      "[!] Max number of coexisting species reached, breaking loop " %in%
        the_function_said
    )
  })
})
